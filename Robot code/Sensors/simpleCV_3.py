...
sp.call(["uvcdynctrl -d '"+camera_id+"' -s 'Exposure (Absolute)'0"], shell = True)    # Set absolute exposure
...
def search():
    ...
    img = cam.getImage()
    objective = img.colorDistance(color=(130,95,160)).invert()
    seg_objective = objective.stretch(150,255)
...
while display.isNotDone():
    ...
    frameTimes[-1] = millis() - lastFrame
    lastFrame = millis()
    frameRate = 1.0 / (sum(frameTimes) / len(frameTimes)) * 1000.0
    
    print "framerate:",frameRate
    
    presearch = millis()
    target = search()
    print "Search took",millis()-presearch,"ms"
